...
static int32_t prv_get_donut_angle(void) {
  // We can draw the donut on either the seconds hand or the minutes hand, depending on the config
  const SprinklesConfiguration *configuration = sprinkles_configuration_get_configuration();
  const int time_unit_of_interest = configuration->seconds_hand_enabled ?
                                        s_app_data->current_seconds :
